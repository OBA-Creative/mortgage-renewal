...
export async function POST(request) {
  try {
    ...
    const { sourceProvinceCode, targetProvinceCodes, rates } =
      await request.json();
    console.log("Rental multiple update request received:", {
      sourceProvinceCode,
      targetProvinceCodes,
    ...
    console.log("Found existing rental rates document:", existingRates._id);
    ...
    console.log(
      "Final rental rate update object keys:",
    ...
    if (!updatedRates) {
      return NextResponse.json(
        { success: false, message: "Failed to update rental rates document" },
        { status: 500 }
      );
    }

    console.log("Multiple rental rate provinces updated successfully");
    ...
    return NextResponse.json({
      success: true,
      message: `Source province ${sourceProvinceCode} and ${targetProvinceCodes.length} target rental rate provinces updated successfully`,
      sourceProvince: sourceProvinceCode,
      targetProvinces: targetProvinceCodes,
      data: updatedData,
    });
  } catch (error) {
    console.error("Error updating multiple rental rate provinces:", error);
    console.error("Error stack:", error.stack);
    return NextResponse.json(
      {
        success: false,
        message: "Failed to update multiple rental rate provinces",
        error: error.message,
      },
      { status: 500 }
    );
